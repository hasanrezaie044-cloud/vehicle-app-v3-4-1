...
export default function FinanceScreen() {
  ...
  const {
    ...
    addLoan, updateLoan, deleteLoan, toggleLoanInstallmentPaid,
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
                  ...
                  return (
                    <View key={loan.id} style={[styles.loanCard, { backgroundColor: colors.surface, borderColor: colors.border }]}>
                      <TouchableOpacity style={styles.loanHeaderRow} onPress={() => setExpandedLoanId(expanded ? null : loan.id)} activeOpacity={0.75}>
                        ...
                        <TouchableOpacity onPress={() => setEditingLoan(loan)} hitSlop={8}>
                          <Ionicons name="create-outline" size={18} color={colors.accent} style={{ marginLeft: 6 }} />
                        </TouchableOpacity>
      ...
      <LoanFormModal
        visible={showLoanModal || !!editingLoan}
        editingLoan={editingLoan}
        onClose={() => { setShowLoanModal(false); setEditingLoan(null); }}
        onSubmit={(data) => {
          if (editingLoan) { updateLoan(editingLoan.id, data); } else { addLoan(data); }
        }}
...
function LoanFormModal({ visible, editingLoan, onClose, onSubmit, getTodayJalali, formatNumber }: {
  visible: boolean;
  ...
  useEffect(() => {
    if (editingLoan) {
      ...
      setInterestPercent(String(editingLoan.interestPercent || 0));
      setInstallmentsCount(String(editingLoan.installmentsCount));
    } else {
      setTitle('');
      ...
      <View style={[modalStyles.overlay, { backgroundColor: colors.overlay }]}>
        <View style={[modalStyles.sheet, { backgroundColor: colors.surface }]}>
          <View style={[modalStyles.header, { borderBottomColor: colors.borderSoft }]}>
            ...
            <Text style={[modalStyles.title, { color: colors.text }]}>{editingLoan ? 'ویرایش وام' : 'ثبت وام جدید'}</Text>
            <View style={{ width: 24 }} />
          </View>
          ...
          >
              ...
              <TouchableOpacity style={[modalStyles.submitBtn, { backgroundColor: colors.accent }]} onPress={handleSubmit}>
                <Text style={[modalStyles.submitBtnText, { color: colors.textOnAccent }]}>{editingLoan ? 'ذخیره تغییرات' : 'ثبت وام'}</Text>
              </TouchableOpacity>
          </KeyboardAwareScrollViewCompat>
        </View>
      </View>
