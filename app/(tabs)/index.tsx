    ...
    <View style={[styles.container, { backgroundColor: colors.bg }]}>
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.content}>
          ...
          <Pressable
            onPress={() => setShowQuickStats(true)}
            style={({ pressed }) => [styles.quickStatsContainer, { backgroundColor: colors.surface, borderColor: colors.border, opacity: pressed ? 0.85 : 1 }]}
          >
            <View style={styles.quickStatsHeader}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.todaySubtitle, { color: colors.textMuted }]}>تعداد سرویس، درآمد، هزینه، کیلومتر و ساعت</Text>
              </View>
          ...
          <QuickStatsSheet
            ...
            privateMetric={privateMetric}
            onSelect={(type) => {
              setShowQuickStats(false);
              router.push({ pathname: '/stats-detail', params: { type, year: String(selectedYear) } });
            }}
          />
        </View>
      </ScrollView>
...
function QuickStat({ onPress, icon, color, soft, value, label, colors }: any) {
  return (
    <Pressable onPress={onPress} style={({ pressed }) => [styles.quickStat, { backgroundColor: colors.surfaceAlt, borderColor: colors.border, opacity: pressed ? 0.85 : 1 }]}>
...
function QuickStatsSheet({ visible, onClose, colors, selectedYear, stats, formatNumber, privateValue, privateMetric, onSelect }: any) {
  return (
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose} statusBarTranslucent>
      <Pressable style={styles.sheetOverlay} onPress={onClose}>
        <Pressable style={[styles.sheetCard, { backgroundColor: colors.surface }]} onPress={() => {}}>
          <View style={styles.sheetHandle} />
          <Text style={[styles.sheetTitle, { color: colors.text }]}>آمار کلی {selectedYear}</Text>
          <View style={styles.quickStatsGrid}>
            <QuickStat onPress={() => onSelect('count')}
            ...
            <QuickStat onPress={() => onSelect('income')}
            ...
            <QuickStat onPress={() => onSelect('expenses')}
            ...
            <QuickStat onPress={() => onSelect('hours')}
...
const styles = StyleSheet.create({
  ...
  quickStatsContainer: {
    marginTop: spacing.sm, borderWidth: 1, borderRadius: radii.lg, padding: spacing.lg,
    ...Platform.select({
  ...
  sectionTitle: { fontSize: 17, fontWeight: '800', textAlign: 'right', writingDirection: 'rtl' },
  sheetOverlay: { flex: 1, backgroundColor: 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' },
  sheetCard: { borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: spacing.lg, paddingBottom: spacing.xl },
  sheetHandle: { width: 40, height: 4, borderRadius: 2, backgroundColor: '#8888', alignSelf: 'center', marginBottom: 16 },
  sheetTitle: { fontSize: 17, fontWeight: '800', textAlign: 'right', marginBottom: 16, writingDirection: 'rtl' },
  quickStatsGrid: { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'space-between', gap: 12 },
  quickStat: {
    borderRadius: radii.lg, padding: 14, width: '48%', alignItems: 'center', borderWidth: 1, gap: 6,
    ...Platform.select({
