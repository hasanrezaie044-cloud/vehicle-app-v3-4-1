...
import { exportReportFiles } from '@/utils/reportExport';
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.bg, paddingTop: topInset }]} contentContainerStyle={styles.contentContainer} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.exportRow}>
        <TouchableOpacity style={[styles.exportBtn,{backgroundColor:colors.surface,borderColor:colors.border}]} onPress={()=>exportReportFiles({services:fServices,fuels:fFuels,maintenances:fMaintenances,periodLabel:period==='year'?'سال جاری':period==='month'?'ماه جاری':'همه'}).catch(()=>{})}>
          <Ionicons name="document-text-outline" size={18} color={colors.accent} /><Text style={[styles.exportBtnText,{color:colors.text}]}>خروجی Excel / CSV</Text>
        </TouchableOpacity>
      </View>
