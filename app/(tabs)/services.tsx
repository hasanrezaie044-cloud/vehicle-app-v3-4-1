...
export default function ServicesScreen() {
  ...
  const {
    ...
    calculateServiceIncome, getServiceCalculationBreakdown, rates, formatNumber, getTodayJalali,
  ...
  const { editId, quick, quickDate } = useLocalSearchParams<{ editId?: string; quick?: string; quickDate?: string }>();
  const [editingId, setEditingId] = useState<string | null>(null);
  // «ثبت سریع سرویس»: میان‌بری به همین فرم کامل است (همان منطق و کامپوننت‌ها)
  // با یک تفاوت: فیلد «شماره درخواست» در این حالت نمایش داده نمی‌شود.
  const isQuickMode = quick === '1';

  const [serviceType, setServiceType] = useState<Service['type']>('night');
  const [date, setDate] = useState(quickDate || getTodayJalali());
  ...
  }, [serviceType, km, hours, tollCount, missionFood, missionToll, missionFine, startTime, endTime, calculateServiceIncome]);

  const calculationBreakdown = useMemo(() => {
    return getServiceCalculationBreakdown(
      serviceType,
      parseFloat(km) || 0,
      parseFloat(hours) || 0,
      parseInt(tollCount) || 0,
  ...
  }, [serviceType, km, hours, tollCount, missionFood, missionToll, missionFine, startTime, endTime, getServiceCalculationBreakdown]);

  const resetForm = useCallback(() => {
    setServiceType('night'); setDate(quickDate || getTodayJalali()); setKm(''); setHours('');
    ...
    setEditingId(null);
    if (editId) router.replace(isQuickMode ? { pathname: '/(tabs)/services', params: { quick: '1', quickDate: quickDate || '' } } : '/(tabs)/services');
  }, [getTodayJalali, editId, router, quickDate, isQuickMode]);
  ...
  const showRequestNumber = !['fixed','mission'].includes(serviceType) && !isQuickMode;
  ...
  const renderForm = () => (
    ...
    >
        <View style={[styles.formHero, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={[styles.formHeroTitle, { color: colors.text }]}>ثبت سریع سرویس</Text>
            <Text style={[styles.formHeroText, { color: colors.textMuted }]}>
              {isQuickMode ? 'همان فرم کامل سرویس؛ فقط بدون شماره درخواست.' : 'اطلاعات را وارد کنید؛ مبلغ خودکار محاسبه می‌شود.'}
            </Text>
          </View>
        </View>
        ...
        {(calculationBreakdown.hourly.nightHours > 0.001 || calculationBreakdown.hourly.saharHours > 0.001) && (
          <View style={[styles.breakdownBox, { backgroundColor: colors.surfaceAlt, borderColor: colors.borderSoft }]}>
            <View style={styles.breakdownHeader}>
              <Ionicons name="analytics-outline" size={14} color={colors.textMuted} />
              <Text style={[styles.breakdownTitle, { color: colors.textSecondary }]}>ریز محاسبه ساعتی</Text>
            </View>
            {calculationBreakdown.hourly.normalHours > 0.001 && (
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownValue, { color: colors.text }]}>{formatNumber(calculationBreakdown.hourly.normalAmount)} ت</Text>
                <Text style={[styles.breakdownLabel, { color: colors.textMuted }]}>{calculationBreakdown.hourly.normalHours.toFixed(2)} ساعت عادی</Text>
              </View>
            )}
            {calculationBreakdown.hourly.nightHours > 0.001 && (
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownValue, { color: colors.night }]}>{formatNumber(calculationBreakdown.hourly.nightAmount)} ت</Text>
                <Text style={[styles.breakdownLabel, { color: colors.textMuted }]}>{calculationBreakdown.hourly.nightHours.toFixed(2)} ساعت شب (+{calculationBreakdown.nightPercent}٪)</Text>
              </View>
            )}
            {calculationBreakdown.hourly.saharHours > 0.001 && (
              <View style={styles.breakdownRow}>
                <Text style={[styles.breakdownValue, { color: colors.mission }]}>{formatNumber(calculationBreakdown.hourly.saharAmount)} ت</Text>
                <Text style={[styles.breakdownLabel, { color: colors.textMuted }]}>{calculationBreakdown.hourly.saharHours.toFixed(2)} ساعت سحرگاه (+{calculationBreakdown.saharPercent}٪)</Text>
              </View>
            )}
          </View>
        )}
        ...
        {!editingId && isQuickMode && router.canGoBack() && (
          <TouchableOpacity style={[styles.cancelButton, { backgroundColor: colors.surfaceAlt }]} onPress={() => router.back()} activeOpacity={0.8}>
            <Text style={[styles.cancelButtonText, { color: colors.textSecondary }]}>بازگشت به تقویم</Text>
          </TouchableOpacity>
        )}
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.bg, paddingTop: topInset }]}>
      <View style={styles.headerRow}>
        <View>
          <Text style={[styles.headerTitle, { color: colors.text }]}>{editingId ? 'ویرایش سرویس' : isQuickMode ? 'ثبت سریع سرویس' : 'ثبت سرویس'}</Text>
          <Text style={[styles.headerSubtitle, { color: colors.textMuted }]}>{isQuickMode ? `ثبت سریع برای تاریخ ${date}` : 'سرویس‌های ثبت‌شده را از تقویم مدیریت کنید'}</Text>
        </View>
        {editingId && (
...
const styles = StyleSheet.create({
  ...
  breakdownBox: { borderRadius: radii.sm, borderWidth: 1, padding: 10, marginTop: 8 },
  breakdownHeader: { flexDirection: 'row-reverse', alignItems: 'center', gap: 6, marginBottom: 6 },
  breakdownTitle: { fontSize: 11.5, fontWeight: '700', writingDirection: 'rtl' },
  breakdownRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 3 },
  breakdownLabel: { fontSize: 11.5, writingDirection: 'rtl' },
