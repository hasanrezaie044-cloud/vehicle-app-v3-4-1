...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const calculateServiceIncomeWithRates = useCallback((
    ...
    switch (serviceType) {
      case 'night':
        income = (km * customRates.nightKm) + (hours * customRates.hour);
        break;
      case 'holiday':
        income = (km * customRates.holidayKm) + (hours * customRates.hour);
        break;
      case 'request':
        income = (km * customRates.fixedRequestKm) + (hours * customRates.hour);
        break;
      case 'available':
        income = hours * customRates.hour;
        break;
      case 'fixed':
        income = (km * customRates.fixedRequestKm) + (hours * customRates.hour);
        break;
  ...
  const calculateServiceIncome = useCallback((
    ...
    return calculateServiceIncomeWithRates(serviceType, km, hours, tollCount, missionFood, missionToll, missionFine, rates);
  }, [rates, calculateServiceIncomeWithRates]);
  ...
  const value = useMemo<AppContextValue>(() => ({
    ...
    changePin, needsPinSetup, completeFirstPinSetup, login, loginWithBiometric, logout, calculateServiceIncome, calculateServiceIncomeWithRates,
